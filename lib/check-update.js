...
export async function checkUpdate(log) {
  ...
  try {
    ...
    exec(`openclaw plugins update memos-cloud-openclaw-plugin`, (error, stdout, stderr) => {
      clearInterval(progressInterval);
      
      const outText = (stdout || "").trim();
      const errText = (stderr || "").trim();
      
      if (outText) log.info?.(`${ANSI.CYAN}[openclaw-cli]${ANSI.RESET}\n${outText}`);
      if (errText) log.warn?.(`${ANSI.RED}[openclaw-cli]${ANSI.RESET}\n${errText}`);
      ...
      setTimeout(() => {
        ...
        if (error || !actuallyUpdated) {
          ...
          log.warn?.(`${ANSI.RED}[memos-cloud] Auto-update failed (${reason}). Please refer to the CLI logs above, or run manually: openclaw plugins update memos-cloud-openclaw-plugin${ANSI.RESET}`);
        } else {
